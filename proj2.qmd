...
eda_sample %>%
  ...
  geom_density(data = eda_sample %>% filter(label == -1), 
               fill = 3, alpha = 0.5) +
  geom_density(data = eda_sample %>% filter(label == 0), 
               fill = 1, alpha = 0.5)
```
...
ggplot(eda_sample %>% filter(label != 0), aes(x=as.factor(label),y=BF, fill=label)) + 
...
# maybe? 
ggplot(eda_sample %>% filter(label != 0), aes(x=as.factor(label),y=log(SD + 0.001), fill=label)) + 
  geom_boxplot(alpha=.7) + 
  theme(legend.position = "none") + 
...
ggplot(eda_sample %>% filter(label != 0), aes(x=as.factor(label),y=NDAI, fill=label)) + 
  geom_boxplot(alpha=.7) + 
  theme(legend.position = "none") + 
...
# CORR 
# good predictor for 1 
ggplot(eda_sample %>% filter(label != 0), aes(x=as.factor(label),y=CORR, fill=label)) + 
  geom_boxplot(alpha=.7) + 
  theme(legend.position = "none") + 
