...
qda1naive = p1qda %>% filter(label == 1) %>%
  summarize("Naive QDA" = 1 - mean(label == test_qda_label1)) #0.128
qda2naive = p1qda %>% filter(label == -1) %>%
  summarize("Naive QDA" = 1 - mean(label == test_qda_label1)) #0.062
...
rbind("Truely Cloud" = cloud, "Truly No Cloud" = nocloud)
```
