...
# maybe? 
ggplot(eda_sample, aes(x=as.factor(label),y=SD, fill=label)) + 
  geom_boxplot(alpha=.7) + 
  theme(legend.position = "none") + 
  labs(title = "Labels vs. BF", 
       x = "-1: No Cloud | 0 : Uncertain | 1 : Cloud",
...
Toy example to illustrate log loss, will put this in CV function for logistic reg
...
CVmaster = function(classifier, train_feat, train_labels, folds=5, loss) {
  ...
  for (i in 1:folds){
    ...
    predictions = predict(classifier,train_feat[val[[i]],],type='response')
    
    # calculate an arbitrary loss function 
    # for example, log loss for logistic regression
    L = loss(train_labels, predictions)
...
Trying to check CV function on mtcars

Using WHOLE thing as validation set

```{r}
fit = glm(vs ~ mpg, data = mtcars, family = 'binomial')
train_features = mtcars[,c("mpg")] # just used mpg as feature 
train_lab = mtcars[,c('vs')] # 0 1 label 


predict(fit, train_features)
