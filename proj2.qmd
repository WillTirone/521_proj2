...
CVmaster = function(classifier, train_feat, train_labels, folds=5, loss) {
  ...
  # creating k folds 
  val = createFolds(train_labels$label, k = folds)
...
# predicting 
glm.prob = predict(fit, train_features, type='response')

# this will be outside of function 
glm.pred = rep(-1,1000)
glm.pred[glm.prob > 0.5] = 1 

# feed this in as loss argument
glm.pred[0:20]

table(glm.pred,train_lab)
...
merge_m1_feat = merge_m1 |> select(NDAI,SD,CORR)
merge_m1_lab = merge_m1 |> select(label)
...
merge_m1_feat = merge_m2 |> select(NDAI,SD,CORR)
merge_m1_lab = merge_m2 |> select(label)
...
glm1.probs <- predict(fit1, naive_test, type = "response")
...
CVmaster(classifier = fit1,
         train_feat = merge_m1_feat, 
...
```{r}
#NAIVE
fit1penalty = glm(label ~ .,
           data = merge_m1, 
           alpha = 1, #lasso
           family='binomial')
glm1.probs <- predict(fit1penalty, naive_test, type = "response")


#ALTERNATE
fit2penalty = glm(label ~ .,
           data = merge_m2, 
           alpha = 1, #lasso 
           family='binomial')
glm1.probs <- predict(fit2penalty, alt_test, type = "response")
```
...
#NAIVE WAY
qda.fit <- qda(label ~ ., data = merge_m1)
qda.class <- predict(qda.fit, naive_test)$class

test_prob = predict(qda.fit, naive_test, type = "response")
test_roc = roc(naive_test$label ~ test_prob, plot = TRUE, print.auc = TRUE)

#WAY NUMBER 2
qda.fit <- qda(label ~ ., data = merge_m2)
qda.class <- predict(qda.fit, alt_test)$class
...
```{r}
#NAIVE
lda.fit <- lda(label ~ ., data = merge_m1)
test_prob = predict(lda.fit, naive_test, type = "response")
test_roc = roc(naive_test$label ~ test_prob, plot = TRUE, print.auc = TRUE)

#ALTERNATIVE
lda.fit <- lda(label ~ ., data = merge_m2)
test_prob = predict(lda.fit, alt_test, type = "response")
test_roc = roc(alt_test$label ~ test_prob, plot = TRUE, print.auc = TRUE)
```
...
library(e1071)
#NAIVE^2
nb.fit <- naiveBayes(label ~ ., data = merge_m1)
nb.preds <- predict(nb.fit, naive_test, type = "raw")

#ALTERNATE
nb.fit <- naiveBayes(label ~ ., data = merge_m2)
nb.preds <- predict(nb.fit, alt_test, type = "raw")
```
